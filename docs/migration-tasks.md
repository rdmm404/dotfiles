...
- [ ] Add `lib/deploy.sh`.
- [ ] Plan the `global/` layer before the current platform layer.
- [ ] List every target link before changing anything.
- [ ] Detect normal files, unrelated links, broken links, and repository-owned links.
- [ ] Treat old links into this repository as safe legacy links that can be replaced.
- [ ] Ensure legacy-link replacement does not require adoption.
- [ ] Refuse all unrelated conflicts during a normal deploy.
- [ ] Verify that platform layers do not replace files owned by `global/`.

### Deployment commands

- [ ] Implement `dot deploy`.
- [ ] Implement `dot deploy --adopt`.
- [ ] Implement `dot undeploy`.
- [ ] Make repeated deployment produce no unnecessary changes.
- [ ] Verify links after deployment.
- [ ] Make undeploy remove only links owned by this repository.
- [ ] Keep undeploy from uninstalling applications or deleting normal files.

### Adoption and backups

- [ ] Add `lib/backups.sh`.
- [ ] Store backups under `~/.local/state/dot/backups/<timestamp>/`.
- [ ] Preserve paths relative to the home directory.
- [ ] Show every conflict and backup destination before adoption.
- [ ] Copy all conflicts successfully before changing any targets.
- [ ] Never call Stow's native `--adopt` option.
- [ ] Implement `dot backups list`.
- [ ] Implement `dot backups restore <timestamp>`.
- [ ] Implement `dot backups remove <timestamp>`.
- [ ] Implement `dot backups prune --older-than <days>d`.
- [ ] Require confirmation for restore, remove, and prune.
- [ ] Refuse unrelated restore conflicts.
- [ ] Keep restored backups until explicitly removed.

### Bootstrap

- [ ] Connect successful installation to deployment in `dot bootstrap`.
- [ ] Show one combined plan before bootstrap changes anything.
- [ ] Ask for confirmation once.
- [ ] Keep bootstrap from adopting files automatically.
- [ ] Run final checks after bootstrap.

### Deployment TDD cycles

Write each failing fake-home test before implementing its behavior:

- [ ] Test a clean fake-home deploy, then implement it.
- [ ] Test a repeated deploy, then make it do no unnecessary work.
- [ ] Test conflict refusal without partial changes, then implement it.
- [ ] Test adoption and backup contents, then implement them.
- [ ] Test backup restoration, then implement it.
- [ ] Test backup removal and age-based pruning, then implement them.
- [ ] Test undeployment, then implement it.
- [ ] Test safe replacement of old repository-owned Stow links, then implement it.
- [ ] Test refusal to remove unrelated links, then implement it.
- [ ] Test command failure midway and prove it leaves no partial changes.

### Phase exit

- [ ] Confirm the full deployment lifecycle passes against a temporary home directory.
- [ ] Confirm a failed preflight leaves the target unchanged.
