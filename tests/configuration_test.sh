...
configuration_deploy_test() {
  ...
  case "$configuration_platform" in
    ubuntu)
      ...
      assert_contains "$configuration_tmp/home/.config/herdr/config.toml" 'Shared personal tmux-style controls' || { rm -rf "$configuration_tmp"; return 1; }
    ...
    omarchy)
      assert_linked "$configuration_tmp/home/.config/herdr/config.toml" || { rm -rf "$configuration_tmp"; return 1; }
      assert_contains "$configuration_tmp/home/.config/herdr/config.toml" 'Generated by dot' || { rm -rf "$configuration_tmp"; return 1; }
      assert_contains "$configuration_tmp/home/.config/herdr/config.toml" 'delivery = "system"' || { rm -rf "$configuration_tmp"; return 1; }
      [ ! -e "$configuration_tmp/home/.config/herdr/config.merge.toml" ] || { rm -rf "$configuration_tmp"; fail 'merge input was deployed'; return 1; }
